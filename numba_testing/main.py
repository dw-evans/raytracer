...
@jit(nopython=True)
# @cuda.jit
def create_from_quaternion(quat: np.ndarray):
...
@jit(nopython=True)
# @cuda.jit
def create_from_axis_rotation(axis: np.ndarray, theta):
    dtype = axis.dtype
...
class Csys:
    ...
    def rotation_matrix(self):
        return create_from_quaternion(self.quat)

    def rzg(self, degrees: float) -> Csys:
        rad = np.radians(degrees)
        ax = np.array([0.0, 0.0, 1.0], dtype=np.float32)
        self.quat = self.quat * create_from_axis_rotation(ax, rad)
        return self
...
pass
# operate_on_csys1(x, COUNT)
# operate_on_csys2(x, COUNT)
...
# @timer
# def operate_on_csys1(csys: Csys, count=1):
...
# @jit(nopython=True)
# def myfunc1():
#     x = np.random.random((1_000, 1_000))
#     for i in range(COUNT):
